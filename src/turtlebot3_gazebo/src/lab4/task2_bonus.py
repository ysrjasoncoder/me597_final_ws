...
class RRTStarLocalPlanner:
    """
    只做局部：输入 (sx,sy)->(gx,gy)，输出 waypoints[(x,y), ...]
    依赖：collision_free(p, q) 由外部注入（解耦）
    依赖：sample bounds 由外部给定（例如围绕局部窗口）
    """
    def __init__(self,
                 collision_free_fn,
        ...
        self.collision_free = collision_free_fn
    ...
    def plan(self,
        ...
        tree: List[RRTNode] = [RRTNode(sx, sy, parent=-1, cost=0.0)]

        def dist(a, b):
            return float(np.hypot(a[0]-b[0], a[1]-b[1]))

        def nearest_idx(x, y):
            d2 = [(n.x - x)**2 + (n.y - y)**2 for n in tree]
            return int(np.argmin(d2))

        def steer(from_xy, to_xy):
            fx, fy = from_xy
            tx, ty = to_xy
            d = np.hypot(tx-fx, ty-fy)
            if d < 1e-9:
                return fx, fy
            s = min(self.step_size, d)
            ux, uy = (tx-fx)/d, (ty-fy)/d
            return fx + ux*s, fy + uy*s

        def near_indices(x, y, r):
            r2 = r*r
            out = []
        ...
        for _ in range(self.max_iter):
            ...
            # 2) nearest + steer
            ni = nearest_idx(rx, ry)
            nx, ny = tree[ni].x, tree[ni].y
            newx, newy = steer((nx, ny), (rx, ry))

            # 3) collision check (edge)
            if not self.collision_free((nx, ny), (newx, newy)):
                continue

            # 4) choose parent (RRT*)
            new_cost = tree[ni].cost + dist((nx, ny), (newx, newy))
            parent = ni

            neigh = near_indices(newx, newy, self.rewire_radius)
            for j in neigh:
                px, py = tree[j].x, tree[j].y
                if not self.collision_free((px, py), (newx, newy)):
                    continue
                cand = tree[j].cost + dist((px, py), (newx, newy))
                if cand < new_cost:
                    new_cost = cand
                    parent = j

            new_idx = len(tree)
            tree.append(RRTNode(newx, newy, parent=parent, cost=new_cost))
            ...
            for j in neigh:
                if j == parent:
                    continue
                px, py = tree[j].x, tree[j].y
                if not self.collision_free((newx, newy), (px, py)):
                    continue
                cand = new_cost + dist((newx, newy), (px, py))
                if cand < tree[j].cost:
                    tree[j].parent = new_idx
                    tree[j].cost = cand

            # 6) check goal
            if dist((newx, newy), (gx, gy)) <= self.goal_tolerance:
                # 尝试直接连到 goal（更稳定）
                if self.collision_free((newx, newy), (gx, gy)):
                    goal_cost = new_cost + dist((newx, newy), (gx, gy))
                    if goal_cost < best_goal_cost:
                        best_goal_cost = goal_cost
                        # 追加一个 goal 节点
                        best_goal_idx = len(tree)
                        tree.append(RRTNode(gx, gy, parent=new_idx, cost=goal_cost))
...
class PathFinder():
    ...
    def local_replan(self, current_pose, global_path, lookahead_distance=3.0):
        ...
        # 3) 检查这段是否有动态障碍（沿用你原逻辑：只要点落在 dyn 就算）
        has_obstacle = False
        ...
        self.node.get_logger().info(f'Obstacle detected, local RRT* replanning from idx {current_idx} to {lookahead_idx}')

        # 4) RRT* 局部规划：start = 当前位姿, goal = lookahead waypoint
        goal_pose = global_path.poses[lookahead_idx]
        gx, gy = goal_pose.pose.position.x, goal_pose.pose.position.y

        # 4.1 给局部采样边界：围绕 start-goal 的包围盒 + margin（米）
        margin = 1.0
        xmin = min(current_x, gx) - margin
        xmax = max(current_x, gx) + margin
        ymin = min(current_y, gy) - margin
        ymax = max(current_y, gy) + margin

        # 可选：再夹紧到地图 limits（你 Map 有 self.limits）
        # lim = self.mp.map.limits  # [xmin,xmax,ymin,ymax]
        # xmin = max(xmin, lim[0]); xmax = min(xmax, lim[1])
        # ymin = max(ymin, lim[2]); ymax = min(ymax, lim[3])

        # 4.2 注入碰撞检测（利用 Navigation 的 _segment_hits_obstacle）
        # 注意：PathFinder 里拿不到 Navigation 的方法，所以把 collision_fn 作为参数传进来更“解耦”
        collision_free = getattr(self.node, "_collision_free", None)
        if collision_free is None:
            # 兜底：用你原来 A* 方式
            self.node.get_logger().warn("No collision_free callback found, fallback to A* local")
            return global_path

        planner = RRTStarLocalPlanner(
            collision_free_fn=collision_free,
            step_size=0.25,
        ...
        if not waypoints or len(waypoints) < 2:
            self.node.get_logger().warn('Local RRT* failed, keeping original path')
            return global_path

        # 5) waypoints -> Path（并融合尾部全局路径）
        new_path = Path()
        ...
        for (x, y) in waypoints:
            new_path.poses.append(self.build_a_pose(x, y))

        # 拼接全局尾巴（去掉重复的 lookahead 点）
        new_path.poses.extend(global_path.poses[lookahead_idx+1:])

        self.node.get_logger().info(f'Local RRT* success: {len(waypoints)} local + {len(global_path.poses)-(lookahead_idx+1)} global')
        return new_path
...
# 常见 scan frame: "base_scan" / "laser" / msg.header.frame_id
    ...
    def _collision_free(self, a_xy, b_xy) -> bool:
        # True 表示可通行
        return (not self._segment_hits_obstacle(a_xy, b_xy, step=0.05))
